-- Fix All Analytics RLS Policies (Final)
...
-- Step 2: Completely disable RLS on all analytics tables
...
-- Step 3: Re-enable RLS on all analytics tables
...
-- Step 4: Create permissive policies for all analytics tables
...
-- Step 6: Check for any triggers that might cause issues
SELECT 
    trigger_name,
    event_manipulation,
    event_object_table,
...
-- Step 8: Verify all policies are working
SELECT schemaname, tablename, policyname, permissive, roles, cmd, qual, with_check 
FROM pg_policies 
