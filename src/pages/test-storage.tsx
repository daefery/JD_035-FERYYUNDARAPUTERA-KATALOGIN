...
export default function TestStorage() {
  ...
  const testStorageAccess = async () => {
    ...
    try {
      ...
      setTestResult(`✅ List successful. Found ${listData?.length || 0} files in logos folder`);
  ...
  const checkBucketPolicies = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        setTestResult(`✅ Bucket accessible. Found ${data?.length || 0} items in root`);
      }
    } catch (error) {
      ...
      <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="mt-6 p-4 bg-yellow-50 border border-yellow-200 rounded">
            <h3 className="font-semibold text-yellow-800 mb-2">If you get RLS errors:</h3>
            <ol className="text-sm text-yellow-700 space-y-1">
              <li>1. Go to your Supabase Dashboard</li>
              <li>2. Go to Storage → Policies</li>
              <li>3. Run the SQL from <code>database/storage_policies.sql</code></li>
              <li>4. Make sure buckets are set to "Public" for read access</li>
            </ol>
          </div>
        </div>
      </div>
